...
|------|-------------|------|---------|:--------:|
| <a name="input_channel"></a> [channel](#input\_channel) | Name of Windows event log channel (eg. Microsoft-Windows-PowerShell/Operational). It expects a single channel name. To collect multiple channels, add multiple integrations. | `string` | n/a | yes |
...
| <a name="input_fleet_package_version"></a> [fleet\_package\_version](#input\_fleet\_package\_version) | Version of the winlog package to use. Use "latest" to resolve the latest available version from the package registry at plan time. | `string` | `"2.6.0"` | no |
| <a name="input_forwarded"></a> [forwarded](#input\_forwarded) | Indicates if this will be consuming forwarded events. If `true` the integration will assume the events  were not generated in the host. This MUST be set to `true` if the integration is consuming forwarded events like in WEF setups, otherwise the integration could run in a degraded state due to inconsistencies in the events correlation. If the channel being consumed is `ForwardedEvents` this is going to be set to `true` regardless of the value set here. | `bool` | `false` | no |
...
| <a name="input_level"></a> [level](#input\_level) | A list of event levels to include. The value is a comma-separated list of levels. Accepted levels are: `critical`, `error`, `warning`, `information`, and `verbose`. | `string` | `null` | no |
...
| <a name="input_xml_query"></a> [xml\_query](#input\_xml\_query) | Provide a custom XML query. This option is mutually exclusive with the `name`, `event_id`, `ignore_older`, `level`, and `providers` options. These options should be included in the XML query directly. Furthermore, an id must be provided. Custom XML queries provide more flexibility and advanced options than the simpler query options. | `string` | `null` | no |

...
| <a name="input_cursor_yaml"></a> [cursor\_yaml](#input\_cursor\_yaml) | Configuration for incremental data collection. Supported keys: enabled, column, type, default, direction. See the SQL Input documentation for details. | `string` | `"enabled: false\ncolumn: id\ntype: integer\ndefault: \"0\"\n"` | no |
...
| <a name="input_fleet_package_version"></a> [fleet\_package\_version](#input\_fleet\_package\_version) | Version of the sql package to use. Use "latest" to resolve the latest available version from the package registry at plan time. | `string` | `"1.2.0"` | no |
...
| <a name="input_sql_queries_yaml"></a> [sql\_queries\_yaml](#input\_sql\_queries\_yaml) | One or more SQL queries to run each collection period, each with its own response\_format (variables or table). This is the standard collection mode. Ignored when a Cursor Query is set. | `string` | `"- query: SHOW GLOBAL STATUS LIKE 'Innodb_system%'\n  response_format: variables\n        \n"` | no |
| <a name="input_sql_query"></a> [sql\_query](#input\_sql\_query) | A single SQL query for cursor-based incremental fetching, used together with Cursor Configuration (enabled: true). When set, it replaces SQL Queries and always runs with "table" response format. Include exactly one :cursor placeholder in the WHERE clause and an ORDER BY on the cursor column. | `string` | `""` | no |

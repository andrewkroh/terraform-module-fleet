...
| <a name="input_assume_role_duration"></a> [assume\_role\_duration](#input\_assume\_role\_duration) | Duration of the assumed role session. | `string` | `null` | no |
| <a name="input_assume_role_expiry_window"></a> [assume\_role\_expiry\_window](#input\_assume\_role\_expiry\_window) | Amount of time before the assumed role session expires during which credentials are refreshed. | `string` | `null` | no |
| <a name="input_aws_region"></a> [aws\_region](#input\_aws\_region) | AWS Region. | `string` | n/a | yes |
| <a name="input_batch_size"></a> [batch\_size](#input\_batch\_size) | The maximum number of evaluation results returned on each page. You cannot specify a number greater than 100. | `number` | `100` | no |
...
| <a name="input_fleet_package_version"></a> [fleet\_package\_version](#input\_fleet\_package\_version) | Version of the aws package to use. Use "latest" to resolve the latest available version from the package registry at plan time. | `string` | `"7.0.0"` | no |
...
| <a name="input_interval"></a> [interval](#input\_interval) | Interval to fetch AWS Config Findings from AWS. | `string` | `"24h"` | no |
| <a name="input_preserve_duplicate_custom_fields"></a> [preserve\_duplicate\_custom\_fields](#input\_preserve\_duplicate\_custom\_fields) | Preserve aws.config fields that were copied to Elastic Common Schema (ECS) fields. | `bool` | `false` | no |
...
| <a name="input_rate_limit"></a> [rate\_limit](#input\_rate\_limit) | The maximum number of API requests per second to send to AWS Config. AWS Config APIs have rate limits that vary by operation. A conservative default of 1 request per second helps avoid ThrottlingException errors. Supported time units are s (per second). | `string` | `"1"` | no |
| <a name="input_rate_limit_burst"></a> [rate\_limit\_burst](#input\_rate\_limit\_burst) | The maximum burst of requests that can be made before rate limiting kicks in. This allows short bursts of requests while still respecting the overall rate limit. Default is 1 to be conservative and avoid throttling. | `number` | `1` | no |
| <a name="input_retry_max_attempts"></a> [retry\_max\_attempts](#input\_retry\_max\_attempts) | The maximum number of retries for a failed API request due to throttling or transient errors. Uses exponential backoff between retries. | `number` | `5` | no |
| <a name="input_retry_wait_max"></a> [retry\_wait\_max](#input\_retry\_wait\_max) | The maximum time to wait before retrying a failed request. Supported time units are s/m/h. | `string` | `"60s"` | no |
| <a name="input_retry_wait_min"></a> [retry\_wait\_min](#input\_retry\_wait\_min) | The minimum time to wait before retrying a failed request. Supported time units are s/m/h. | `string` | `"1s"` | no |
...
| <a name="input_supports_identity_federation"></a> [supports\_identity\_federation](#input\_supports\_identity\_federation) | n/a | `bool` | `null` | no |
| <a name="input_tags"></a> [tags](#input\_tags) | n/a | `list(string)` | <pre>[<br>  "forwarded",<br>  "aws-config"<br>]</pre> | no |

...
| <a name="input_include_alerts"></a> [include\_alerts](#input\_include\_alerts) | Include associated alerts with the incident. When enabled, the alerts are split so that each generated event represents a single alert from the incident. If disabled, enable the Alerts (alerts\_v2) data stream to ensure alert data is still collected. | `bool` | `true` | no |
...
| <a name="input_tags"></a> [tags](#input\_tags) | Tags for the data-stream. | `list(string)` | <pre>[<br>  "forwarded",<br>  "m365_defender-incident"<br>]</pre> | no |

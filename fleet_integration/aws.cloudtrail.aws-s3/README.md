...
| <a name="input_actor_target_mapping"></a> [actor\_target\_mapping](#input\_actor\_target\_mapping) | Maps and classifies actor and target entity identifiers by type. Actor entities populate `actor.entity.id` and type-specific fields (`user.entity.id`, `service.entity.id`, `host.entity.id`, `entity.id`). Target entities populate `target.entity.id` and corresponding type-specific fields (`user.target.entity.id`, `service.target.entity.id`, `host.target.entity.id`, `entity.target.id`). All identifiers are captured in the `related.entity` field. This Enables enhanced entity visualization in Security Solution. Additional ingest pipeline processors may introduce performance overhead. | `bool` | `true` | no |

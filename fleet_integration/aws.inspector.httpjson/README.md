...
| <a name="input_http_client_timeout"></a> [http\_client\_timeout](#input\_http\_client\_timeout) | Duration of the time limit on HTTP requests. Valid time units are ns, us, ms, s, m, h. | `string` | `"30s"` | no |
| <a name="input_initial_interval"></a> [initial\_interval](#input\_initial\_interval) | How far back to pull the Amazon Inspector Findings from AWS. Supported units for this parameter are h/m/s. | `string` | `"2160h"` | no |
| <a name="input_interval"></a> [interval](#input\_interval) | Interval to fetch Amazon Inspector Findings from AWS. Supported units for this parameter are h/m/s. | `string` | `"24h"` | no |
| <a name="input_lag_time"></a> [lag\_time](#input\_lag\_time) | Safety margin subtracted from the poll cursor to tolerate delay between when Amazon Inspector sets a finding's `updatedAt` and when the finding becomes available from the API. This prevents findings from being missed when there is propagation lag, at the cost of re-fetching a small overlap window on each poll. Re-fetched findings are de-duplicated on ingest, so no duplicate documents are created. Supported units for this parameter are h/m/s. | `string` | `"5m"` | no |
| <a name="input_preserve_duplicate_custom_fields"></a> [preserve\_duplicate\_custom\_fields](#input\_preserve\_duplicate\_custom\_fields) | Preserve aws.inspector fields that were copied to Elastic Common Schema (ECS) fields. | `bool` | `false` | no |

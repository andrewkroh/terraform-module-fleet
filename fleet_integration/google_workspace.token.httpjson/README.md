...
| <a name="input_application_name_for_url"></a> [application\_name\_for\_url](#input\_application\_name\_for\_url) | Do not modify. The application name used to build the endpoint URL. | `string` | `"token"` | no |
...
| <a name="input_preserve_duplicate_custom_fields"></a> [preserve\_duplicate\_custom\_fields](#input\_preserve\_duplicate\_custom\_fields) | Preserve google\_workspace.token fields that were copied to Elastic Common Schema (ECS) fields. | `bool` | `false` | no |
...
| <a name="input_tags"></a> [tags](#input\_tags) | n/a | `list(string)` | <pre>[<br>  "forwarded",<br>  "google_workspace-token"<br>]</pre> | no |

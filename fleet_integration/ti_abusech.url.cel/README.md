...
| <a name="input_fleet_package_version"></a> [fleet\_package\_version](#input\_fleet\_package\_version) | Version of the ti\_abusech package to use. | `string` | `"3.5.0"` | no |
...
| <a name="input_interval"></a> [interval](#input\_interval) | Duration between requests to the URLhaus API. Supported units for this parameter are h/m/s. Example `24h`. As data dump is generated every 5 minutes, it should be greater than 5 minutes. | `string` | `"1h"` | no |
...
| <a name="input_url"></a> [url](#input\_url) | Base URL of the abuse.ch URLhaus API to collect actively distributing malware URLs. | `string` | `"https://urlhaus.abuse.ch/downloads/json"` | no |

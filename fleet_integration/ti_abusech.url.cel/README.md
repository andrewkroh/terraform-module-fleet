...
| <a name="input_fleet_package_version"></a> [fleet\_package\_version](#input\_fleet\_package\_version) | Version of the ti\_abusech package to use. Use "latest" to resolve the latest available version from the package registry at plan time. | `string` | `"4.1.1"` | no |
...
| <a name="input_interval"></a> [interval](#input\_interval) | Duration between requests to the URLhaus API. The API returns at most 1000 URLs added in the last 3 days, so the interval must be short enough to avoid exceeding that limit. Too long an interval risks missing URLs; too short an interval re-collects the same URLs without benefit. Supported units: h/m/s. Example `10m`. | `string` | `"10m"` | no |
| <a name="input_ioc_expiration_duration"></a> [ioc\_expiration\_duration](#input\_ioc\_expiration\_duration) | Indicator is expired after this duration since it was first seen. Supported units for this parameter are d/h/m. Example `10d`. Default value is `90d` i.e., 90 days. | `string` | `"90d"` | no |
...
| <a name="input_tags"></a> [tags](#input\_tags) | n/a | `list(string)` | <pre>[<br>  "forwarded",<br>  "abusech-url"<br>]</pre> | no |
| <a name="input_url"></a> [url](#input\_url) | Base URL of the abuse.ch URLhaus API to collect recently added malware URLs. | `string` | `"https://urlhaus-api.abuse.ch/v1/urls/recent/"` | no |
